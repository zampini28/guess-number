...
def load_training_data(dataset_dir='dataset'):
    dataset, labels = zip(*[
        ...
        for filename in os.listdir(dataset_dir) if filename.endswith('.npy')
        for data in [np.load(os.path.join(dataset_dir, filename), allow_pickle=True).item()]
    ])

    return np.array(dataset), np.array(labels)

def preprocess_data(x):
    return x.astype('float32').reshape(x.shape[0], -1)
